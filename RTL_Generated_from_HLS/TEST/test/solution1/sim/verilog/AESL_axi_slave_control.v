...
//------------------------Parameter----------------------
`define TV_OUT_OutModel0 "../tv/rtldatafile/rtl.CNN.autotvout_OutModel0.dat"
parameter ADDR_WIDTH = 5;
parameter DATA_WIDTH = 32;
parameter OutModel0_DEPTH = 1;
reg [31 : 0] OutModel0_OPERATE_DEPTH = 0;
parameter OutModel0_c_bitwidth = 32;
parameter START_ADDR = 0;
parameter CNN_continue_addr = 0;
parameter CNN_auto_start_addr = 0;
parameter OutModel0_data_out_addr = 16;
parameter OutModel0_valid_out_addr = 20;
...
reg  BREADY_reg = 0;
reg [DATA_WIDTH - 1 : 0] mem_OutModel0 [OutModel0_DEPTH - 1 : 0] = '{default : 'h0};
reg [DATA_WIDTH-1 : 0] image_mem_OutModel0 [ (OutModel0_c_bitwidth+DATA_WIDTH-1)/DATA_WIDTH * OutModel0_DEPTH -1 : 0] = '{default : 'hz};
reg OutModel0_read_data_finish;
...
//read OutModel0 reg
reg [31 : 0] read_OutModel0_count = 0;
reg read_OutModel0_run_flag = 0;
reg read_one_OutModel0_data_done = 0;
...
assign TRAN_control_read_data_finish = 1 & OutModel0_read_data_finish;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        OutModel0_read_data_finish <= 0;
        read_OutModel0_run_flag <= 0; 
        read_OutModel0_count = 0;
        count_operate_depth_by_bitwidth_and_depth (OutModel0_c_bitwidth, OutModel0_DEPTH, OutModel0_OPERATE_DEPTH);
    end
    else begin
        if (AESL_done_index_reg === 1) begin
            read_OutModel0_run_flag = 1; 
        end
        if (TRAN_control_transaction_done_in === 1) begin
            OutModel0_read_data_finish <= 0;
            read_OutModel0_count = 0; 
        end
        if (read_one_OutModel0_data_done === 1) begin
            read_OutModel0_count = read_OutModel0_count + 1;
            if (read_OutModel0_count == OutModel0_OPERATE_DEPTH) begin
                read_OutModel0_run_flag <= 0; 
                OutModel0_read_data_finish <= 1;
            end
        end
    end
end

initial begin : read_OutModel0
    integer read_OutModel0_resp;
    integer process_num;
    integer get_vld;
    ...
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = OutModel0_c_bitwidth;
    process_num = 2;
    count_c_data_four_byte_num_by_bitwidth (c_bitwidth , four_byte_num) ;
    while (1) begin
        process_2_finish <= 0;
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            if (read_OutModel0_run_flag === 1) begin
                process_busy = 1;
                get_vld = 0;
                //read OutModel0 vld
                read (OutModel0_valid_out_addr, RDATA_reg, read_OutModel0_resp);
                ...
                if (get_vld == 1) begin
                    ...
                    for (i = 0 ; i < four_byte_num ; i = i+1) begin
                        read (OutModel0_data_out_addr + read_OutModel0_count * four_byte_num * 4 + i * 4, RDATA_reg, read_OutModel0_resp);
                        if (OutModel0_c_bitwidth < 32) begin
                            mem_OutModel0[read_OutModel0_count] <= RDATA_reg;
                        end
                        else begin
                            for (j=0 ; j < 32 ; j = j + 1) begin
                                if (i*32 + j < OutModel0_c_bitwidth) begin
                                    mem_OutModel0[read_OutModel0_count][i*32 + j] <= RDATA_reg[j];
                                end
                            end
                        end
                    end
                    
                    read_one_OutModel0_data_done <= 1;
                    @(posedge clk);
                    read_one_OutModel0_data_done <= 0;
                end    
                process_busy = 0;
            end    
            process_2_finish <= 1;
...
initial begin : write_OutModel0_file_proc 
  integer fp; 
  integer factor; 
  integer transaction_idx; 
  reg [OutModel0_c_bitwidth - 1 : 0] tmp_cache_mem; 
  reg [ 100*8 : 1] str;
  reg [63:0] bin_data;
  integer i; 
  transaction_idx = 0; 
  count_seperate_factor_by_bitwidth (OutModel0_c_bitwidth , factor);
  while(1) begin 
      @(posedge clk);
      while (OutModel0_read_data_finish !== 1 || TRAN_control_transaction_done_in === 1) begin
          @(posedge clk);
      end
      # 0.1;
      fp = $fopen(`TV_OUT_OutModel0, "a"); 
      if(fp == 0) begin       // Failed to open file 
          $display("Failed to open file \"%s\"!", `TV_OUT_OutModel0); 
          $finish; 
      end 
      $fdisplay(fp, "[[transaction]] %d", transaction_idx);
      for (i = 0; i < (OutModel0_DEPTH - OutModel0_DEPTH % factor); i = i + 1) begin
          if (factor == 4) begin
              if (i%factor == 0) begin
                  tmp_cache_mem = mem_OutModel0[i/factor][7:0];
              end
              if (i%factor == 1) begin
                  tmp_cache_mem = mem_OutModel0[i/factor][15:8];
              end
              if (i%factor == 2) begin
                  tmp_cache_mem = mem_OutModel0[i/factor][23:16];
              end
              if (i%factor == 3) begin
                  tmp_cache_mem = mem_OutModel0[i/factor][31:24];
              end
              $fdisplay(fp,"0x%x",tmp_cache_mem);
          end
          if (factor == 2) begin
              if (i%factor == 0) begin
                  tmp_cache_mem = mem_OutModel0[i/factor][15:0];
              end
              if (i%factor == 1) begin
                  tmp_cache_mem = mem_OutModel0[i/factor][31:16];
              end
              $fdisplay(fp,"0x%x",tmp_cache_mem);
          end
          if (factor == 1) begin
              $fdisplay(fp,"0x%x",mem_OutModel0[i]);
          end
      end 
      if (factor == 4) begin
          if ((OutModel0_DEPTH - 1) % factor == 2) begin
              $fdisplay(fp,"0x%x",mem_OutModel0[OutModel0_DEPTH / factor][7:0]);
              $fdisplay(fp,"0x%x",mem_OutModel0[OutModel0_DEPTH / factor][15:8]);
              $fdisplay(fp,"0x%x",mem_OutModel0[OutModel0_DEPTH / factor][23:16]);
          end
          if ((OutModel0_DEPTH - 1) % factor == 1) begin
              $fdisplay(fp,"0x%x",mem_OutModel0[OutModel0_DEPTH / factor][7:0]);
              $fdisplay(fp,"0x%x",mem_OutModel0[OutModel0_DEPTH / factor][15:8]);
          end
          if ((OutModel0_DEPTH - 1) % factor == 0) begin
              $fdisplay(fp,"0x%x",mem_OutModel0[OutModel0_DEPTH / factor][7:0]);
          end
      end
      if (factor == 2) begin
          if ((OutModel0_DEPTH - 1) % factor == 0) begin
              $fdisplay(fp,"0x%x",mem_OutModel0[OutModel0_DEPTH / factor][15:0]);
          end
      end
