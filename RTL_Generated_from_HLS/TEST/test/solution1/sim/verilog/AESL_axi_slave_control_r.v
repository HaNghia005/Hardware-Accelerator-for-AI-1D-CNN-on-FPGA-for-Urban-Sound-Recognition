...
//------------------------Parameter----------------------
`define TV_IN_InModel "../tv/cdatafile/c.CNN.autotvin_InModel.dat"
`define TV_IN_Weights "../tv/cdatafile/c.CNN.autotvin_Weights.dat"
parameter ADDR_WIDTH = 6;
parameter DATA_WIDTH = 32;
parameter InModel_DEPTH = 1;
reg [31 : 0] InModel_OPERATE_DEPTH = 0;
parameter InModel_c_bitwidth = 64;
parameter Weights_DEPTH = 1;
reg [31 : 0] Weights_OPERATE_DEPTH = 0;
parameter Weights_c_bitwidth = 64;
parameter InModel_data_in_addr = 16;
parameter Weights_data_in_addr = 28;
...
reg  BREADY_reg = 0;
reg [InModel_c_bitwidth - 1 : 0] mem_InModel [InModel_DEPTH - 1 : 0] = '{default : 'h0};
reg [DATA_WIDTH-1 : 0] image_mem_InModel [ (InModel_c_bitwidth+DATA_WIDTH-1)/DATA_WIDTH * InModel_DEPTH -1 : 0] = '{default : 'hz};
reg InModel_write_data_finish;
reg [Weights_c_bitwidth - 1 : 0] mem_Weights [Weights_DEPTH - 1 : 0] = '{default : 'h0};
...
//write InModel reg
reg [31 : 0] write_InModel_count = 0;
reg write_InModel_run_flag = 0;
reg write_one_InModel_data_done = 0;
//write Weights reg
...
assign TRAN_control_r_write_data_finish = 1 & InModel_write_data_finish & Weights_write_data_finish;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        InModel_write_data_finish <= 0;
        write_InModel_run_flag <= 0; 
        write_InModel_count = 0;
        count_operate_depth_by_bitwidth_and_depth (InModel_c_bitwidth, InModel_DEPTH, InModel_OPERATE_DEPTH);
    end
    else begin
        ...
        if (AESL_ready_reg === 1) begin
            write_InModel_run_flag <= 1; 
            write_InModel_count = 0;
        end
        if (write_one_InModel_data_done === 1) begin
            write_InModel_count = write_InModel_count + 1;
            if (write_InModel_count == InModel_OPERATE_DEPTH) begin
                write_InModel_run_flag <= 0; 
                InModel_write_data_finish <= 1;
            end
        end
    end
end

initial begin : write_InModel
    integer write_InModel_resp;
    integer process_num ;
    integer get_ack;
    ...
    integer i;
    integer j;
    reg [31 : 0] InModel_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = InModel_c_bitwidth;
    process_num = 0;
    ...
    while (1) begin
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            get_ack = 1;
            if (write_InModel_run_flag === 1 && get_ack === 1) begin
                process_busy = 1;
                //write InModel data 
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    if (InModel_c_bitwidth < 32) begin
                        InModel_data_tmp_reg = mem_InModel[write_InModel_count];
                    end
                    else begin
                        for (j=0 ; j<32 ; j = j + 1) begin
                            if (i*32 + j < InModel_c_bitwidth) begin
                                InModel_data_tmp_reg[j] = mem_InModel[write_InModel_count][i*32 + j];
                            end
                            else begin
                                InModel_data_tmp_reg[j] = 0;
                            end
                        end
                    end
                    if(image_mem_InModel[write_InModel_count * four_byte_num  + i]!==InModel_data_tmp_reg) begin
                    write (InModel_data_in_addr + write_InModel_count * four_byte_num * 4 + i * 4, InModel_data_tmp_reg, write_InModel_resp);
                    image_mem_InModel[write_InModel_count * four_byte_num + i]=InModel_data_tmp_reg;
                    end
                end
                process_busy = 0;
                write_one_InModel_data_done <= 1;
                @(posedge clk);
                write_one_InModel_data_done <= 0;
            end   
            process_0_finish <= 1;
        end
        @(posedge clk);
...
// Read data from file 
initial begin : read_InModel_file_process 
  integer fp; 
  integer ret; 
  integer factor; 
  reg [151 : 0] token; 
  reg [151 : 0] token_tmp; 
  //reg [InModel_c_bitwidth - 1 : 0] token_tmp; 
  ...
  count_seperate_factor_by_bitwidth (InModel_c_bitwidth , factor);
  fp = $fopen(`TV_IN_InModel ,"r"); 
  if(fp == 0) begin                               // Failed to open file 
      $display("Failed to open file \"%s\"!", `TV_IN_InModel); 
      $finish; 
  end 
  read_token(fp, token); 
  ...
  while (token != "[[[/runtime]]]") begin 
      ...
      for(i = 0; i < InModel_DEPTH; i = i + 1) begin 
          read_token(fp, token); 
          ...
          if (factor == 4) begin
              ...
              if (i%factor == 3) begin
                  ...
                  mem_InModel [i/factor] = tmp_cache_mem;
                  tmp_cache_mem [DATA_WIDTH - 1 : 0] = 0;
              end
          end
          if (factor == 2) begin
