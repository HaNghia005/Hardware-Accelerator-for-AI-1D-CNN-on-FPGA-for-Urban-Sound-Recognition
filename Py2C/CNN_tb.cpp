...
int main(){
	fxp OutModel0;
	fxp* Weights = (fxp*)malloc(62999 * sizeof(fxp));
	float tmp;
	FILE* Weight = fopen("Float_Weights.txt", "r");
	for (int i = 0; i < 62999; i++){
		fscanf(Weight, "%f", &tmp);
		*(Weights + i)=tmp;
	}
	fclose(Weight);
	////read Input
	fxp* InModel = (fxp*)malloc((NumberOfPicture * d * 173 * 40) * sizeof(fxp));
	FILE* Input = fopen("X.txt", "r");
	for (int i = 0; i < NumberOfPicture * d * 173 * 40; i++){
		fscanf(Input, "%f", &tmp);
		*(InModel + i)=tmp;
	}
	fclose(Input);
	...
	for (int i = 0; i < NumberOfPicture ; i++)
	{
		int startIndex = i * d * 173 * 40;
		for (int k = 0; k < d * 173 * 40; k++)
		{
